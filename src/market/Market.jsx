    ...
    case "1D":
      ...
      cells = addIndex(map)(
        (cell, i) => (
          <g
            key={`cell-bottom-${i}`}
            transform={`translate(${i * data.width} ${
              data.stock.title === false ? 0 : 50
            })`}
          >
            <Cell cell={cell} {...pass} />
          </g>
    ...
    case "1Diag":
      ...
      cells = addIndex(map)(
        (cell, i) => (
          <g
            key={`cell-bottom-${i}`}
            transform={`translate(${i * 0.5 * data.width} ${
              i % 2 === 0
                ? data.stock.title === false
                  ? 0
                  : 50
                : (data.stock.title === false ? 0 : 50) + data.height
            })`}
          >
            <Cell cell={cell} {...pass} />
          </g>
      ...
      cells = addIndex(chain)((row, y) => {
        return addIndex(map)(
          (cell, x) => (
            <g
              key={`cell-bottom-${x}-${y}`}
              transform={`translate(${x * data.width} ${
                y * data.height + (data.stock.title === false ? 0 : 50)
              })`}
            >
              <Cell cell={cell} {...pass} />
            </g>
          ...
          {addIndex(map)((legend, i) => {
            ...
            return (
              <g
                key={`pool-note-${i}`}
                transform={`translate(${current} ${
                  2 * data.height + (data.stock.title === false ? 25 : 75)
                })`}
              >
                <Legend {...legend} />
              </g>
